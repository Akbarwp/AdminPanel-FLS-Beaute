...
class ManageReport extends Controller
{
            ...
            $output .= '
                <td class="col-2">
                    <div class="col text-left">
                        ...
                        <button type="button" class="btn btn-primary" style="background-color:rgba(165,78,182); color:whitesmoke" onclick="window.open(`' . $direct . '`)">
    ...
    public function getDateTransactionBuy(Request $request)
    {
        ...
        foreach ($transactions as $transaction) {
            if ($owner->user_position == "superadmin_pabrik") {
                ...
                $distributor = $transaction->nama_supplier;
            } else {
                $code = $transaction->transaction_code;
                $distributor = $transaction->distributor->firstname . ' ' . $transaction->distributor->lastname;
            }
    ...
    // TRANSACTION RETUR BUY
    public function indexTransactionReturBuy(Request $request)
    {
        $from = Carbon::create($request->min);
        $to = Carbon::create($request->max)->endOfDay();
        $owner = auth()->user();
        ...
        return view('manage_report.retur.buy.index', compact(['returs', 'owner']));
    }

    // public function getDateTransactionReturBuy(Request $request)
    // {
    ...
    // public function printTransactionReturBuy(Request $request)
    // {
    ...
    // public function printDetailTransactionReturBuy($id, $type)
    // {
    //     // dd($type);
    //     if (auth()->user()->id_group == 1) {
    //         $owner = User::where('id_group', auth()->user()->id_group)->where('user_position', 'superadmin_pabrik')->first();
    ...
    // TRANSACTION RETUR SELL
    public function indexTransactionReturSell(Request $request)
    {
        $from = Carbon::create($request->min);
        $to = Carbon::create($request->max)->endOfDay();
        $username = auth()->user()->firstname . ' ' . auth()->user()->lastname;
        if (auth()->user()->id_group == 1) {
            $owner = User::where('user_position', 'superadmin_distributor')->first();
            if ($request->min != "" && $request->max != "") {
                $returs = ReturHistory::where('id_approve', auth()->user()->id)->whereBetween('updated_at', [$from, $to])
                    ->select('*')
                    ->get();
            } else if ($request->min == "" && $request->max == "") {
                $returs = ReturHistory::where('id_approve', auth()->user()->id)->whereMonth('updated_at', Carbon::now()->month)->get();
            } else if ($request->min == "") {
                $returs = ReturHistory::where('id_approve', auth()->user()->id)->whereDate('updated_at', '<=', $to)->get();
            } else if ($request->max == "") {
                $returs = ReturHistory::where('id_approve', auth()->user()->id)->whereDate('updated_at', '>=', $from)->get();
            }
            return view('manage_report.retur.sell.index', compact('returs'));
        } else if (auth()->user()->user_position == "superadmin_distributor") {
            $seller = User::where('id_group', auth()->user()->id_group)
                ->where('user_position', 'reseller')
                ->first();
            if ($request->min != "" && $request->max != "") {
                $returs = ReturHistory::where('id_owner', $seller->id)->where('nama_approve', $username)->whereBetween('updated_at', [$from, $to])
                    ->select('*')
                    ->get();
                $returs2 = ReturCashierHistory::where('id_owner', auth()->user()->id)->whereBetween('updated_at', [$from, $to])
                    ->get();
            } else if ($request->min == "" && $request->max == "") {
                $returs = ReturHistory::where('id_owner', $seller->id)->where('nama_approve', $username)->whereMonth('updated_at', Carbon::now()->month)
                    ->get();
                $returs2 = ReturCashierHistory::where('id_owner', auth()->user()->id)
                    ->whereMonth('updated_at', Carbon::now()->month)
                    ->get();
            } else if ($request->min == "") {
                $returs = ReturHistory::where('id_owner', $seller->id)->where('nama_approve', $username)->whereDate('updated_at', '<=', $to)
                    ->get();
                $returs2 = ReturCashierHistory::where('id_owner', auth()->user()->id)
                    ->where('id_group', auth()->user()->id_group)
                    ->whereDate('updated_at', '<=', $to)
                ...
                $returs = ReturHistory::where('id_owner', $seller->id)->where('nama_approve', $username)->whereDate('updated_at', '>=', $from)
                    ->get();
                $returs2 = ReturCashierHistory::where('id_owner', auth()->user()->id)
                    ->where('id_group', auth()->user()->id_group)
                    ->whereDate('updated_at', '>=', $from)
            ...
            //     ->first();
            if ($request->min != "" && $request->max != "") {
                $returs = ReturCashierHistory::where('id_owner', auth()->user()->id)
        ...
        // if (auth()->user()->user_position == "reseller") {
        // }
        return view('manage_report.retur.sell.index', compact(['returs', 'returs2']));
    }
    ...
    public function indexUser()
    {
        if (auth()->user()->lihat_laporan_pegawai == 1) {
            if (auth()->user()->id_group == 1) {
                ...
                $pegawais = User::where('id_group', auth()->user()->id_group)->where('user_position', '!=', 'prospek_reseller')->get();
            }
            ...
            foreach ($pembelians as $pembelian) {
                $output .= "
                    <tr>
                        ...
                        <td scope='col'>" . $pembelian->distributor->firstname . ' ' . $pembelian->distributor->lastname . "</td>
                        <td scope='col'>Rp. " . number_format($pembelian->total, 0, ',', '.') . "</td>
                    </tr>";
            }
    ...
    public function getPenjualanUser()
    {
        ...
        if (auth()->user()->id_group != 1) {
            ...
            foreach ($jualKasirs as $jualKasir) {
                $customer = $jualKasir->nama_pembeli;
                $output .= "
                    <tr>
    ...
    public function getDateAkun(Request $request)
    {
        ...
        $to = Carbon::create($request->max)->endOfDay();
        if (auth()->user()->id_group == 1) {
            $users = User::where('user_position', '!=', 'prospek_distributor')->orWhere('user_position', 'superadmin_distributor');
            if ($request->min != "" && $request->max != "") {
