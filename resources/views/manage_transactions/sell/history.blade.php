...
<div class="container justify text-center">
    <div class="d-flex justify-content-between">
        <div class="container">
            ...
            <form action="{{ url('/manage_transaction/sell/history/print/') }}" method="post" target="_blank">
                @csrf
                <div class="row mt-3 d-flex flex-row">
                    <div class="col-sm d-flex align-items-end justify-content-center">
                        ...
                        <button type="button" class="btn btn-primary form-group mr-3" onclick="window.location.href='{{ url('manage_transaction/sell/history/export/') }}'" style="background-color:rgba(165,78,182); color:whitesmoke">
                ...
                <table id="logTable" class="table table-hover table-light">
                    ...
                    <tbody>
                        @php
                            $totalAll = 0;
                        @endphp
                        @foreach($histories as $item => $history)
                        @php
                        ...
                        <tr id="{{ $history->id }}">
                            ...
                            <td>{{ $details[$item]->lokasi_barang }}</td>
                            <td>{{ $history->updated_at->format('d/m/y H:i:s') }}</td>
                            <td>{{ $history->metode_pembayaran }}</td>
                            <td class="col-2">
                                <div class="col text-left">
                                    <button type="button" class="btn btn-primary" style="background-color:rgba(165,78,182); color:whitesmoke" onclick="window.open('{{ url('/manage_transaction/sell/history/print_detail/'.$history->id) }}')">
...
$('#search').click(function () {
    ...
    $.ajax({
        type:'get',
        url:'{{ url("/manage_transaction/sell/history/get_date") }}',
        data: {
            "_token": "{{ csrf_token() }}",
            min:min,
            max:max        
...
function printHistory()
{
    ...
    var max = document.getElementById("max").value;
    console.log(min);
    $link = "{{ url('/manage_transaction/sell/history/print/'" +min;
    window.open($link);
}
</script>
@endsection