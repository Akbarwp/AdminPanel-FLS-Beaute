            ...
            <div class="container">
                ...
                <div class="row align-items-center form-group">
                    ...
                    {{-- <div class="col-md-6">
                        ...
                        <button type="button" class="btn btn-primary" style="background-color:rgba(165,78,182); color:whitesmoke" onclick="window.location.href='{{ url('/retur/to_cashier/exportXlsx/'.auth()->user()->id) }}'">
                            <i class="fa fa-file-excel-o"></i>
                            <span>Export</span>
                        </button>
        ...
        <div class="col-12 grid-margin">
            ...
            <div class="col text-left" id="logTableDiv">
                    <table id="logTable" class="table table-hover table-striped table-light table-responsive text-left text-nowrap" >
                        ...
                        <tbody>
                            @if(auth()->user()->user_position =='superadmin_pabrik'|| auth()->user()->user_position == 'superadmin_distributor' || auth()->user()->user_position == 'reseller')
                            @foreach($returs as $retur)
                                <tr>
                                    ...
                                    @else
                                    {{-- <td scope="col">{{ $retur->supplier->firstname }} {{ $retur->supplier->lastname }} </td> --}}
                                    @endif
                            ...
                            @foreach($returs2 as $retur2)
                                <tr>
                                    @if(auth()->user()->user_position == "sales")
                                    <td scope="col">{{ $retur2->sales_stok->updated_at->format('d/m/y H:i:s') }}</td>
                                    @else
                                    <td scope="col">{{ $retur2->transaction->transaction_code }}</td>
                                    @endif
                                    <td scope="col">
                                        {{ $retur2->transaction->nama_pembeli }} 
                                    </td>
                                    {{-- {{ $retur2->supplier->firstname }} {{ $retur2->supplier->lastname }}  --}}
                                    @if(auth()->user()->user_position == 'superadmin_pabrik')
                                    <td scope="col">{{ $retur2->surat_keluar }}</td>
                                    @endif
                                    <td scope="col">{{ $retur2->updated_at->format('d/m/y H:i:s') }}</td>
                                    <td scope="col">{{ $retur2->keterangan }}</td>
                                    <td scope="col">Retur Sukses</td>
                                    <td>
                                        <div class="form-group" >
                                            <button type="button" style="background-color:rgba(165,78,182); color:whitesmoke" onclick="location.href='{{ url('/retur/to_cashier/detail/'.$retur2->id) }}'" class="btn btn-primary">
                                                <i class="fa fa-eye"></i>
                                            </button>
                                        </div>
                                    </td>
...
<div class="modal fade" id="importExcel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form method="post" action="{{route('returCashier.import')}}" enctype="multipart/form-data">
